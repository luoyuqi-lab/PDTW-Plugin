...
def LB_NP_EA(a,b,w,ub,lb,N,seg_num,bsf):
    if w >= 1 and len(a) >= 6:
    ...
    for j in range(N):
        ...
        if j < seg_num:
            for i in range(start, stop):
                if lb_PN + lb_K_temp > bsf:
                    return lb_PN + lb_K_temp
                ai = a[i]
                if ai > ub[i]:
                    lb_K_temp += (ai - ub[i]) ** 2
                elif ai < lb[i]:
            ...
            for i in range(start, stop):
                ...
                if ai > ub[i]:
                    ...
                    left_bound = max(0, i - w)
                    right_bound = min(len(b), i + w + 1)
                    min_distance = float('inf')  # reset min_distance
                    for m in range(left_bound, right_bound):
                        distance = (ai - b[m]) ** 2
                        if distance < min_distance:
                            min_distance = distance
    ...
    for p in range(seg_num):
        ...
        for i in range(0, m):
            ...
            if i > w+1:
                ...
                if lb_PN + lb_pdtw > bsf:
                    return lb_PN + lb_pdtw
        lb_PN = lb_PN + lb_pdtw
    return lb_PN
